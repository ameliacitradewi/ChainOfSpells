...
//  ChainOfSpells
//
//  Created by Mushafa Fadzan Andira on 09/05/25.
//
...
// MARK: - GameScene
class FirstTutorialScene: SKScene {
    // Predetermined Value
    ...
    var enemy1 = EnemyModel( name: "enemy_1", folderName: "BossIdle", idleAnimations:  ["boss1","boss2","boss3","boss4","boss5","boss6","boss7"], hp: 10,initialChainEffect: nil, loseImage: "boss-lose")
    ...
    private func updateTutorialStage(){
        ...
        case .takeDamage: do {
            playerHpNode.zPosition = 100
            playerHpLabel.zPosition = 100
            showBlackOverlay()
            tooltipBackground.isHidden = false
            characterSprite.isHidden = false
            characterSprite.position = CGPoint(x:frame.maxX - 200,y : frame.midY)
            tooltipBackground.position = CGPoint(x: playerHpNode.position.x + 200, y: playerHpNode.position.y + 30)
            toolTipLabel.typeText(
...
//        momentumLabel.position = CGPoint(x: momentumBar.position.x + 35, y: frame.minY + 117)
        ...
        comboBackground.zPosition = 10
        comboInfoLabel.zPosition = 11
...
//            .wait(forDuration: replacementDur + postReplaceBuffer),
    ...
    private func gotoNextLevel() {
        ...
        dialogLabel.typeText("If I want to be stronger I must unlock all the elements")
